...
# -------------------- Main --------------------
def main():
    rospy.init_node("molecular_demo_tx_right")
    ...
    # Right robot waits after its calibration while left robot calibrates
    WAIT_AFTER_CALIBRATION = float(p("wait_after_calibration", 135.0))
    ...
    # -------------------- RIGHT ROBOT CALIBRATION --------------------
    pub_state.publish(f"STATE=RIGHT_CALIBRATION_START repeats={PRE_SET_REPEATS}")
    rospy.loginfo("[Right calibration] Sending %d calibration pulses...", PRE_SET_REPEATS)
    ...
    for i in range(int(PRE_SET_REPEATS)):
        ...
        rospy.loginfo("[Right calibration] Pulse %d/%d", i + 1, PRE_SET_REPEATS)
        pub_state.publish(
            f"STATE=RIGHT_CALIBRATION_SPRAY i={i+1}/{PRE_SET_REPEATS} "
        ...
        if i < int(PRE_SET_REPEATS) - 1:
            rospy.loginfo(
                "[Right calibration] Waiting %.1f s before next pulse",
                PRE_WAIT_290,
            )
            pub_state.publish(f"STATE=RIGHT_CALIBRATION_WAIT t={PRE_WAIT_290:.1f}")

            if not sleep_while_running(PRE_WAIT_290, pub_state=pub_state):
                continue

    pub_state.publish("STATE=RIGHT_CALIBRATION_DONE")
    rospy.loginfo("[Right calibration] Calibration pulses done.")

    # Wait while the left robot performs its calibration
    pub_state.publish(f"STATE=WAIT_FOR_LEFT_CALIBRATION t={WAIT_AFTER_CALIBRATION:.1f}")
    rospy.loginfo(
        "[Right robot] Waiting %.1f s for left robot calibration...",
        WAIT_AFTER_CALIBRATION,
    )

    if not sleep_while_running(WAIT_AFTER_CALIBRATION, pub_state=pub_state):
        handle_pause(ser, cmd_pub, tw_stop, pub_state)
    ...
    while not rospy.is_shutdown():
        ...
        # Right robot spray
        rospy.loginfo("[Right Spray] %.1f s", LOOP_SPRAY_10)
        pub_state.publish(f"STATE=RIGHT_LOOP_SPRAY t={LOOP_SPRAY_10:.1f}")

        if not spray_step(ser, LOOP_SPRAY_10, pub_state=pub_state):
            continue

        # Wait before left robot sprays
        rospy.loginfo("[Right Wait before left spray] %.1f s", LOOP_WAIT_290)
        pub_state.publish(f"STATE=RIGHT_WAIT_BEFORE_LEFT_SPRAY t={LOOP_WAIT_290:.1f}")

        if not sleep_while_running(LOOP_WAIT_290, pub_state=pub_state):
            continue

        # Wait during left spray + wait after left spray
        extra_wait = LOOP_SPRAY_10 + LOOP_WAIT_290

        rospy.loginfo("[Right Wait during left spray and after] %.1f s", extra_wait)
        pub_state.publish(
            f"STATE=RIGHT_WAIT_DURING_LEFT_SPRAY_AND_AFTER t={extra_wait:.1f}"
        )

        if not sleep_while_running(extra_wait, pub_state=pub_state):
            continue

        # Move
        pub_state.publish(f"STATE=MOVE t={MOVE_TIME:.1f}")
        rospy.loginfo("[Right Move] %.1f seconds", MOVE_TIME)
